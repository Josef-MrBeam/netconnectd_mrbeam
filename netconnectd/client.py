...
from netconnectd.protocol import (
    StartApMessage,
    StopApMessage,
    ListWifiMessage,
...
def client_configure_wifi(address, args):
    client_send_message(
        address, ConfigureWifiMessage(ssid=args.ssid, psk=args.psk, force=args.force)
    )
...
def client():
    ...
    subparser = subparsers.add_parser(
        "version", help="Display version information and exit"
    )
    ...
    subparser.add_argument(
        ...
        "--force",
        action="store_true",
        help="Force refresh of the wifi list, might cause a short disconnect if currently in AP mode",
    )
    ...
    subparser.add_argument(
        ...
        "--force",
        action="store_true",
        help="Force refresh of the wifi list while configuring, might cause a short disconnect if currently in AP mode",
    )
    ...
    # forget_wifi parser
    subparser = subparsers.add_parser(
        "forget_wifi", help="Forgets the configured WIFI connection"
    )
    ...
    if config["socket"] is None:
        parser.error(
            "Socket address is missing, please provide either via --address or via --configfile"
        )
